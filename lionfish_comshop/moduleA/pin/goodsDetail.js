...
Page({
  ...
  authSuccess: function () {
    ...
    let url = '/lionfish_comshop/moduleA/pin/goodsDetail?id=' + id + '&scene=' + scene;
      ...
      success: function (res) {
        if (res.data.code == 3) {
          ...
          if (pages_all.length > 3) {
            wx.redirectTo({
              url: `/lionfish_comshop/pages/order/placeOrder?type=${buy_type}&is_limit=${is_limit}`
            })
          } else {
            wx.navigateTo({
              url: `/lionfish_comshop/pages/order/placeOrder?type=${buy_type}&is_limit=${is_limit}`
  ...
  onShareAppMessage: function () {
    ...
    var share_path = `lionfish_comshop/moduleA/pin/goodsDetail?id=${goods_id}&share_id=${share_id}&community_id=${community_id}`;
