...
Page({
  ...
  onShow: function () {
    let that = this;
    let id = this.options && this.options.id || 0;
    id && this.getData(id), this.getCommentList();
  ...
  changeCartNum: function(e){
    let { goods_total_count, total, goods_id } = e.detail;
  ...
  showCartGoods: function (showModal=true){
    ...
    return new Promise(function (resolve, reject) {
      const token = wx.getStorageSync('token');
      let soli_id = that.data.soli_info.id || '';
      let currentCommunity = wx.getStorageSync('community');
      ...
      soli_id && wx.showLoading(), app.util.request({
        ...
        success: function (res) {
          ...
          if (res.data.code == 0) {
            ...
            let emptyCarts = false;
            if (Object.keys(carts).length == 0){
              reject();
            } else {
              showModal && that.setData({ showCartModal: true, carts })
              resolve();
            }
  ...
  changeCart: function(){
    let id = this.options && this.options.id || 0;
    id && this.setData({ clearTime: true }), this.getData(id);
  },
    ...
    app.util.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          that.setData({ soli_info, list, content: '', showCommentModal: false })
  ...
  onShareAppMessage: function () {
    ...
    let share_path = `lionfish_comshop/moduleA/solitaire/details?id=${soli_info.id}&share_id=${share_id}`;
    return {
      title,
      path: share_path,
      success: function (res) {
