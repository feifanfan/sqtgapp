...
Page(Object.assign({}, countDownInit.default, {
  ...
  handleOpenSubcribeMesage:function(){
    ...
    wx.requestSubscribeMessage({
      tmplIds: ['CBHk9t7BP7JsC5R-Xagu49yXyqoH2IRfjjezuLd_yLE','l1hJ4h0wLY28cMiLby4tDEsydo5AhT9LTi6opt-tubw'],
      success(res) {
        console.log('订阅消息');
        console.log(res);
      },
    ...
    app.util.request({
      ...
      success: function(res) {
        ...
        if (rdata.code == 0) {
          ...
          //秒杀
          let { scekill_time_arr, seckill_bg_color, seckill_is_open, seckill_is_show_index, hide_community_change_word,index_qgtab_counttime } = rdata;
          let myDate = new Date();
  ...
  classificationChange: function(t) {
    ...
    this.$data = {...this.$data, ...{
      ...
      if (!this.$data.stickyFlag && (that.$data.scrollTop != that.$data.stickyTop+5)){
        wx.pageScrollTo({ scrollTop: that.$data.stickyTop - 30, duration: 0 })
      }
      that.load_goods_data();
    });
  },
  ...
  */
  ...
  onPullDownRefresh: function () {
    this.setData({ couponRefresh: false, newComerRefresh: false });
    this.tpage = 1;
    this.$data = {
      ...this.$data, ...{
        overPageNum: 1,
  ...
  */
  ...
  qiandao_handler:function(){
      wx.navigateTo({
        url:'/lionfish_comshop/moduleA/score/signin',
      });
  },
  ...
  getPinList: function () {
    ...
    app.util.request({
      url: 'entry/wxapp/index',
      data: { controller: 'group.get_pintuan_list', is_index: 1 },
      dataType: 'json',
      success: function (res) {
        if (res.data.code == 0) {
          let pinList = {};
