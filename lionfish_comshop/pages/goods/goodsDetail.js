...
Page({
  ...
  onLoad: function(options) {
    ...
    if (options.community_id != 'undefined' && options.community_id > 0 && buy_type!='integral') {
      ...
      // 没有分享社区直接访问
      this.get_goods_details(options.id, '', currentCommunity_id);
      h.community = currentCommunity;
    }
  ...
  get_goods_details: function (id, communityInfo='', currentCommunity_id){
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        // 会员
        let { is_need_subscript, need_subscript_template, is_open_vipcard_buy, modify_vipcard_name, is_vip_card_member, modify_vipcard_logo, is_member_level_buy } = res.data;
