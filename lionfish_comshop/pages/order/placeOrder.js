...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.showLoading()
    console.log('pickUpCommunityName:' + this.data.pickUpAddress+'  '+this.data.pickUpCommunityName);
        ...
        success: function (res) {
          ...
          param.tabAddress = [
            ...
            { 
              ...
              receiverAddress: that.data.pickUpAddress + '  ' + that.data.pickUpCommunityName,//edit by ff tuan_send_address_info.address || '选择位置', 
  ...
  goOrderfrom: function() {
    ...
    if (tabIdx == 2){ this.prepay(); } else { this.conformOrder(); }
  },
  ...
  changeReceiverName: function(e) {
    ...
    tabAddress[tabIdx].name = receiverName;
    if (!receiverName) {
      let tip = '请填写收货人';
      if (tabIdx == 0) tip = '请填写提货人';
      wx.showToast({
  ...
  choseLocation: function() {
    ...
    wx.chooseLocation({
      success: function (e) {
        ...
        function setRes(){
          console.log('setData');
          console.log('tabIdx:'+tabIdx+'-filename:'+filename);
  ...
  calcPrice: function(isTabSwitch = 0){
    let tdata = this.data;
    let { total_free, delivery_tuanz_money, trans_free_toal, tabIdx, goods, is_open_vipcard_buy, is_member_level_buy, is_vip_card_member, canLevelBuy } = tdata;
